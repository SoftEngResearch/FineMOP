...
if [[ -z ${RUN_CONFIGS} ]]; then
  RUN_CONFIGS=("test" "MOP" "VMS" "STARTS" "CLASSES_ps1c" "CLASSES_ps3cl" "FINE_ps1c" "FINE_ps3cl" "HyRTS_ps1c" "HyRTS_ps3cl" "HyRTS-S_ps1c" "HyRTS-S_ps3cl" "METHODS_ps1c" "METHODS_ps3cl" "METHODS-F_ps1c" "METHODS-F_ps3cl" "METHODS-FA_ps1c" "METHODS-FA_ps3cl" "METHODS-S_ps1c" "METHODS-S_ps3cl")
else
...
function execute {
  ...
  local variant="$(echo "$command" | cut -d "_" -f 2)"
  (
    ...
    if [[ ${command} != "test" && ${command} != "compile" ]]; then
      ...
      if [[ ${command} == "VMS" || ${command} == "MOP" || ${command} == "STARTS" ]]; then
    ...
    if [ "${command}" == "test" ]; then
      time mvn test -fae ${SKIPS} -Dmaven.repo.local=${project_repo}
    elif [ "${command}" == "compile" ]; then
      time mvn clean compile -fae ${SKIPS} -Dmaven.repo.local=${project_repo}
    elif [[ "${command}" == "HyRTS-S"* ]]; then
      time mvn -Dmaven.ext.class.path=${ENV_DIR}/extensions/emop-extension-1.0-SNAPSHOT.jar:${ENV_DIR}/extensions/emop-agent-extension-1.0-SNAPSHOT.jar emop:rps -DuseThirdParty=${USE_THIRD_PARTY} -Dgranularity=HYBRID -DfinerSpecMapping=true $(get_variant_flags $variant) ${EMOP_STATS} ${VERBOSE_AGENT} ${DEBUG_TRUE} ${PROFILER_OPTIONS} -fae ${SKIPS} -Dmaven.repo.local=${project_repo} -DjavamopAgent=${project_agent}
    elif [[ "${command}" == "HyRTS"* ]]; then
      time mvn -Dmaven.ext.class.path=${ENV_DIR}/extensions/emop-extension-1.0-SNAPSHOT.jar:${ENV_DIR}/extensions/emop-agent-extension-1.0-SNAPSHOT.jar emop:rps -DuseThirdParty=${USE_THIRD_PARTY} -Dgranularity=HYBRID $(get_variant_flags $variant) ${EMOP_STATS} ${VERBOSE_AGENT} ${DEBUG_TRUE} ${PROFILER_OPTIONS} -fae ${SKIPS} -Dmaven.repo.local=${project_repo} -DjavamopAgent=${project_agent}
    elif [[ "${command}" == "METHODS-FA"* ]]; then
      time mvn -Dmaven.ext.class.path=${ENV_DIR}/extensions/emop-extension-1.0-SNAPSHOT.jar:${ENV_DIR}/extensions/emop-agent-extension-1.0-SNAPSHOT.jar emop:rps -DuseThirdParty=${USE_THIRD_PARTY} -Dgranularity=METHOD -DfinerSpecMapping=true -DfinerInstrumentation=true -DfinerInstrumentationAlt=true $(get_variant_flags $variant) ${EMOP_STATS} ${VERBOSE_AGENT} ${DEBUG_TRUE} ${PROFILER_OPTIONS} -fae ${SKIPS} -Dmaven.repo.local=${project_repo} -DjavamopAgent=${project_agent}
      ...
      time mvn -Dmaven.ext.class.path=${ENV_DIR}/extensions/emop-extension-1.0-SNAPSHOT.jar:${ENV_DIR}/extensions/emop-agent-extension-1.0-SNAPSHOT.jar emop:rps -DuseThirdParty=${USE_THIRD_PARTY} -Dgranularity=METHOD $(get_variant_flags $variant) ${EMOP_STATS} ${VERBOSE_AGENT} ${DEBUG_TRUE} ${PROFILER_OPTIONS} -fae ${SKIPS} -Dmaven.repo.local=${project_repo} -DjavamopAgent=${project_agent}
    elif [[ "${command}" == "CLASSES"* ]]; then
      time mvn -Dmaven.ext.class.path=${ENV_DIR}/extensions/emop-extension-1.0-SNAPSHOT.jar:${ENV_DIR}/extensions/emop-agent-extension-1.0-SNAPSHOT.jar emop:rps -DuseThirdParty=${USE_THIRD_PARTY} -e $(get_variant_flags $variant) ${EMOP_STATS} ${VERBOSE_AGENT} ${DEBUG_TRUE} ${PROFILER_OPTIONS} -fae ${SKIPS} -Dmaven.repo.local=${project_repo} -DjavamopAgent=${project_agent}
    elif [[ "${command}" == "FINE"* ]]; then
      ...
      time mvn -Dmaven.ext.class.path=${ENV_DIR}/extensions/emop-extension-1.0-SNAPSHOT.jar:${ENV_DIR}/extensions/mop-agent-extension-1.0-SNAPSHOT.jar emop:vms -fae ${SKIPS} -Dmaven.repo.local=${project_repo} -DforceSave=true
    elif [ "${command}" == "MOP" ]; then
      time mvn -Dmaven.ext.class.path=${ENV_DIR}/extensions/mop-agent-extension-1.0-SNAPSHOT.jar test -fae ${SKIPS} -Dmaven.repo.local=${project_repo}
    elif [ "${command}" == "STARTS" ]; then
      time mvn -Dmaven.ext.class.path=${ENV_DIR}/extensions/starts-extension-1.0-SNAPSHOT.jar:${ENV_DIR}/extensions/mop-agent-extension-1.0-SNAPSHOT.jar starts:starts -fae ${SKIPS} -Dmaven.repo.local=${project_repo}
...
function main {
  ...
  (
    ...
    while read -r version; do
      ...
      mkdir -p "${data_path}"
      # The next line is authored by Kevin Guan:
      ...
      for algorithm in "${RUN_CONFIGS[@]}"; do
        ...
        if [[ ${iteration} != 1 ]]; then mv ".${algorithm}" ".${STARTS}" &> "${DEV_NULL}"; fi
        touch "NEW"
